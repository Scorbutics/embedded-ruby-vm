...
/**
 * Get the native library directory path from the asset layout.
 * This provides Kotlin with the definitive path to native libraries
 * managed by C code.
 *
 * @param env JNI environment
 * @param clazz Class object (unused)
 * @param install_dir_jstr Installation directory path
 * @return String path to native libs directory, or NULL on error
 */
JNIEXPORT jstring JNICALL
Java_com_scorbutics_rubyvm_RubyVMNative_getNativeLibsPath(JNIEnv *env, jclass clazz,
                                                          jstring install_dir_jstr) {
    (void) clazz;

    if (!install_dir_jstr) {
        jni_log_write(JNI_LOG_ERROR, "RubyVM", "getNativeLibsPath: install_dir is NULL");
        return NULL;
    }

    // Convert jstring to C string
    char* install_dir = jstring_to_cstring(env, install_dir_jstr);
    if (!install_dir) {
        jni_log_write(JNI_LOG_ERROR, "RubyVM", "getNativeLibsPath: failed to convert install_dir");
        return NULL;
    }
    ...
    AssetsLayout* layout = assets_get_layout(install_dir, &error);

    if (!layout) {
        jni_log_printf(JNI_LOG_ERROR, "RubyVM",
                      "getNativeLibsPath: Failed to get asset layout: %s", error.message);
        free(install_dir);
        return NULL;
    }
 ...
 */
JNIEXPORT jint JNICALL
Java_com_scorbutics_rubyvm_RubyVMNative_getNativeLibCount(JNIEnv *env, jclass clazz,
                                                          jstring install_dir_jstr) {
    (void) clazz;
...
/**
 * Get a specific native library path by index.
 *
 * @param env JNI environment
 * @param clazz Class object (unused)
 * @param install_dir_jstr Installation directory path
 * @param index Index of the library to get
 * @return Full path to the library file, or NULL if index is invalid
 */
JNIEXPORT jstring JNICALL
Java_com_scorbutics_rubyvm_RubyVMNative_getNativeLibPath(JNIEnv *env, jclass clazz,
                                                         jstring install_dir_jstr,
                                                         jint index) {
    (void) clazz;

    if (!install_dir_jstr || index < 0) {
        return NULL;
    }
